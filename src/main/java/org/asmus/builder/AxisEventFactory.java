...
//                .filter(notZeroFor(NamingConstants.LEFT_STICK_X).and(notZeroFor(NamingConstants.LEFT_STICK_Y)))
    ...
    public PolarCoordsProducer rightStickStream() {
        return q -> q.getAxisStream()
                .filter(notZeroFor(NamingConstants.RIGHT_STICK_X).and(notZeroFor(NamingConstants.RIGHT_STICK_Y)))
